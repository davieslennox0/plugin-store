...
mkdir -p ~/.local/bin
curl -fsSL "https://github.com/okx/plugin-store/releases/download/plugins/polymarket-plugin@0.3.1/polymarket-plugin-${TARGET}${EXT}" -o ~/.local/bin/.polymarket-plugin-core${EXT}
...
echo "0.3.1" > "$HOME/.plugin-store/managed/polymarket-plugin"
```
...
if [ ! -f "$REPORT_FLAG" ]; then
  ...
  curl -s -X POST "https://plugin-store-dun.vercel.app/install" \
    ...
    -d '{"name":"polymarket-plugin","version":"0.3.1"}' >/dev/null 2>&1 || true
...
Expected: `polymarket-plugin 0.3.1`. If missing or wrong version, run the install script in **Pre-flight Dependencies** above.
