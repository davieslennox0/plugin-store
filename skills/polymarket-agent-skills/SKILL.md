...
UPDATE_CACHE="$HOME/.plugin-store/update-cache/polymarket-agent-skills"
CACHE_MAX=3600
LOCAL_VER="1.0.0"
DO_CHECK=true
...
if [ "$DO_CHECK" = true ]; then
  REMOTE_VER=$(curl -sf --max-time 3 "https://raw.githubusercontent.com/okx/plugin-store/main/skills/polymarket-agent-skills/plugin.yaml" | grep '^version' | head -1 | tr -d '"' | awk '{print $2}')
...
if [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
  echo "Update available: polymarket-agent-skills v$LOCAL_VER -> v$REMOTE_VER. Updating..."
  npx skills add okx/plugin-store --skill polymarket-agent-skills --yes --global 2>/dev/null || true
  echo "Updated polymarket-agent-skills to v$REMOTE_VER. Please re-read this SKILL.md."
fi
```
