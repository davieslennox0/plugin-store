...
| 我想要... | 类型 |
|-----------|------|
| 使用 AI agent 通过自然语言实现特定任务，比如调用 onchainos 命令创建策略 | 仅 Skill |
...
当你的源代码位于你自己的 GitHub 仓库时，使用 `repo` 和 `commit` 代替 `dir`：
...
#### Binary Plugin 示例（Skill + 编译后的 CLI）
...
#### 二进制 Plugin 本地源码（不外链仓库）

如果源码直接在 Plugin 目录中（不在单独的仓库），省略 `source_repo` 和 `source_commit`：
...
此时 CI 直接从 `skills/my-rust-tool/` 编译——你的 `Cargo.toml`、`src/main.rs` 等必须和 `SKILL.md`、`plugin.yaml` 在同一目录。
...
- **保留前缀**：`okx-`、`official-`、`plugin-store-` —— 只有 OKX 组织成员可以使用 `okx-`

---

## 5. 编写 SKILL.md

SKILL.md 是你的 Plugin 的**唯一入口**。它教 AI 代理你的 Plugin 做什么以及如何使用它。对于仅 Skill 的 Plugin，它编排 onchainos 命令。对于 Binary Plugin，它还编排你的自定义工具。
...
你的 SKILL.md 告诉 AI 代理如何使用 **onchainos 命令**和你的**自定义 binary 工具**：
...
5. **不要重复 onchainos 功能** —— 编排现有命令，而不是替换它们
...
**适用场景**：你愿意将所有源代码直接放在 plugin-store 仓库中。适合仅 Skill 的 Plugin 和包含少量脚本的 Plugin。
...
[OnChain OS](https://github.com/okx/onchainos-skills) 是 Agentic Wallet CLI，提供安全的沙箱化区块链操作 —— 钱包签名、交易广播、swap 执行、合约调用等。它使用 TEE（可信执行环境）签名，因此私钥永远不会离开安全隔区。
...
- 使用保留的名称前缀（`okx-`、`official-`、`plugin-store-`）
...
可以。修改你的文件，在 `plugin.yaml` 和 `SKILL.md` 中升级 `version`，然后提交一个标题为 `[update] my-plugin v1.1.0` 的新 PR。

**Plugin 的命名规则是什么？**

仅允许小写字母、数字和连字符。2 到 40 个字符之间。不允许连续的连字符。不允许下划线。`okx-` 前缀仅保留给 OKX 组织成员使用。

**我可以使用任何编程语言吗？**

对于 binary Plugin，支持的语言有 Rust、Go、TypeScript（Bun）、Node.js（Bun）和 Python。对于仅 Skill 的 Plugin，你可以包含任何语言的脚本（Python 和 shell 脚本很常见）—— 它们作为 AI 代理工作流的一部分运行，不由 CI 编译。
...
AI 审查仅供参考，不会阻止你的 PR。但是，人工审查员会阅读 AI 报告。解决标记的问题可以加速审批。

**我的本地 lint 通过了但 GitHub 检查失败。为什么？**

确保你运行的是最新版本的 plugin-store CLI。同时确认你的 PR 只修改了 `skills/your-plugin-name/` 内的文件。

**构建在 CI 中失败但本地可以编译。为什么？**

CI 在 Ubuntu Linux 上编译。确保你的代码在 Linux 上可构建，而不仅仅是 macOS 或 Windows。检查 GitHub Actions 运行中的构建日志以获取具体错误。
...
`build.source_commit` 必须是完整的 commit 哈希，不是短 SHA 或分支名。在你的源代码仓库中运行 `git rev-parse HEAD` 获取完整的 40 个字符的哈希。

**错误 E120 "must also include a Skill component" 是什么意思？**

每个有 `build` 部分的 Plugin 必须同时有 SKILL.md。Skill 是入口 —— 它告诉 AI 代理如何使用你的 binary。

**错误 E130 "pre-compiled binary file is not allowed" 是什么意思？**

你在提交目录中包含了编译后的文件（.exe、.dll、.so、.wasm 等）。删除它 —— 我们从你的源代码编译，你不需要提交二进制文件。

**错误 E110/E111 "requires a build section" 是什么意思？**

你声明了 Binary 组件但没有包含 `build` 部分。我们需要知道你的源代码在哪里以便编译。添加 `build.lang`、`build.source_repo` 和 `build.source_commit`。
