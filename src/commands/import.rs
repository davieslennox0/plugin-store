...
"#
    ...
    let community_repo = "okx/plugin-store-community";

    // Fork (idempotent — gh handles already-forked case)
    run_cmd("gh", &["repo", "fork", community_repo, "--clone=false"])?;

    // Clone fork
    let fork_repo = format!("{}/{}", github_user, "plugin-store-community");
    ...
    // Write files
    let sub_dir = format!("{}/submissions/{}", work_dir, name);
    ...
    // Commit and push
    git(&["add", &format!("submissions/{}", name)])?;
